// LogiStar Map — Step 1 (v3 robust + cache-busting)
...
async function boot(){
  try{
    const s = await fetch('data/sectors.json?v=3', {cache:'no-cache'}).then(r=>r.json());
    const y = await fetch('data/systems.json?v=3', {cache:'no-cache'}).then(r=>r.json());
    sectors = s.sectors;
    systems = y.systems;
    ...
    err.innerText = 'Ошибка загрузки данных. Проверьте, что файлы data/sectors.json и data/systems.json существуют в корне gh-pages.';
