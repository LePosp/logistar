...
export function buildVoronoiSectors(sites,border){
  const cells=[];
  for(let i=0;i<sites.length;i++){
    ...
    for(let j=0;j<sites.length;j++){ if(i===j) continue;
      ...
      cell=clipPolyHalfPlane(cell,n,c); if(!cell.length)break;
    }
    if(cell.length){ let cx=0,cy=0; for(const v of cell){cx+=v.x; cy+=v.y;} cx/=cell.length; cy/=cell.length;
