...
export function attach(canvas, st, api){
  ...
  // --- CLICK to select system / cluster ---
  canvas.addEventListener('click', e=>{
    if(st.mode!=='galaxy') return;
    const px=e.clientX, py=e.clientY;
    // 1) Try clusters first (if visible and computed)
    if(st._clusters && st._clusters.length){
      for(const c of st._clusters){
        const dx=px - c.sx, dy=py - c.sy;
        const r=Math.max(10, 8 + Math.log2(4+c.count)*5);
        if(dx*dx+dy*dy <= (r*r)){
          st.onZoomToWorld && st.onZoomToWorld(c.wx,c.wy);
          return;
        }
      }
    }
    // 2) Pick nearest system within pixel radius
    let best=null, bestD=1e9;
    const pr = Math.max(10, 12*st.t.scale); // pick radius in px
    ...
    if(best){
      st.selected = best.sys;
      st.showCard && st.showCard(best.sys);
      draw();
    }else{
      st.selected = null;
      st.hideCard && st.hideCard();
      draw();
    }
  });
