...
export function initUI(st,{draw,toGalaxy,toSystem}){
  ...
  const card=document.getElementById('card'), enter=document.getElementById('enterBtn'), close=document.getElementById('closeCard');
  enter.onclick=()=>{ if(st.selected) toSystem(st.selected.id); };
  close.onclick=()=>{ card.style.display='none'; };
  st.showCard=sys=>{ document.getElementById('cardTitle').textContent=sys.name; document.getElementById('cardMeta').textContent=`Тип: ${sys.type.toUpperCase()} • Коорд: ${Math.round(sys.x)}, ${Math.round(sys.y)}`; card.style.display='block'; };
